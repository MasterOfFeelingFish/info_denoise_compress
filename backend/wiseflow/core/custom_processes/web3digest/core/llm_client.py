...
不要返回其他任何说明文字，只返回索引号。"""
    ...
    async def _generate_scores_and_reason(self, info: Dict, user_profile: str) -> Dict:
        ...
        prompt = f"""你是Web3资讯评分专家。请对这条信息进行多维度评分，区分不同质量等级。
...
- 8-10分（高质量）: 完美匹配用户关注的领域，包含用户关心的关键词
- 4-5分: 部分匹配用户兴趣，有一定相关性
- 1-3分: 相关性较低，与用户兴趣匹配度不高

### 2. 重要性(0-3分): 信息本身的重要程度
- 8-10分（高质量）: 重大事件（如融资、升级、重要公告）、来自权威来源（Vitalik、以太坊官方、Coinbase等）
- 2-3分: 中等重要性，行业动态或项目更新
- 1分: 一般性信息，重要性较低
...
**高质量内容（总分≥8分）的特征**：
- 来自权威来源（VitalikButerin、以太坊官方、CoinDesk、Coinbase等）
- 重大事件（重大融资、重要升级、安全漏洞等）
- 完美匹配用户兴趣领域
- 时效性强（24小时内）
...
**重要**：请根据质量标准给出有区分度的评分，不要全部集中在6-8分。真正高质量的内容应该给8-10分。
...
{{
    ...
    "importance_score": 2.5,
    "freshness_score": 1.8,
    "reason": "匹配您关注的DeFi领域,来自权威来源CoinDesk,讨论了重要升级事件"
}}"""
